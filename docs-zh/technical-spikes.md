...
| S3 无管理员 Windows 部署 | 打包最小 Qt Widgets 窗口／文件选择及微型提取示例。在 Windows 10/11 x64 无开发运行时、无提权条件下验证。优先便携目录，用户级安装仅在 IT 允许时考虑。 | 普通用户可启动／核查／替换／移除；不依赖 UAC、Program Files、机器级写入；依赖随包、数据可写且保留。记录 OS、Python/Qt/打包工具版本、体积、启动、中文路径／输入、DPI、终端策略。断网传入／部署／首次启动／核查必须通过，不使用缓存前置依赖；分别测试保留 Python 3.8 和未装 Python 的机器，记录 PATH／外部运行时依赖。 | 初始 4–8 h，需要 Windows/IT 环境；失败阻塞该分发方式，不自动更换技术栈。 |
